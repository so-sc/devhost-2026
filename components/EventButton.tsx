...
const EventButton = React.forwardRef<HTMLButtonElement, EventButtonProps>(
  (
    ...
    const handleMouseDown  = (e: React.MouseEvent<HTMLButtonElement>) => { setPressed(true);  onMouseDown?.(e); };
    const handleMouseUp    = (e: React.MouseEvent<HTMLButtonElement>) => { setPressed(false); onMouseUp?.(e);   };
    const handleMouseLeave = (e: React.MouseEvent<HTMLButtonElement>) => { setPressed(false); onMouseLeave?.(e); };
    ...
    return (
      ...
      >
        <svg width="0" height="0" className="pointer-events-none absolute">
          <defs>
            <filter id="roughen-ev" x="-5%" y="-5%" width="110%" height="110%">
              <feTurbulence type="fractalNoise" baseFrequency="0.065 0.08" numOctaves="4" seed="12" result="noise" />
        ...
        >
          <div className="absolute inset-0" style={{ clipPath: "url(#event-clip)", background: CHARCOAL_BASE }} />
          <div className="absolute inset-0 opacity-90" style={{ clipPath: "url(#event-clip)", background: SCRATCH_LAYER }} />
          <div className="absolute inset-0" style={{ clipPath: "url(#event-clip)", background: STAIN_LAYER }} />
          <div className="absolute inset-0" style={{ clipPath: "url(#event-clip)", background: PIT_LAYER }} />
          ...
          {pressed && (
            <div
              ...
              style={{
                clipPath: "url(#event-clip)",
                background: "radial-gradient(ellipse at 50% 40%, rgba(0,0,0,0.5) 0%, transparent 80%)",
              }}
            />
          )}
          ...
          {glow && (
            <div
              ...
              style={{
                clipPath: "url(#event-clip)",
                background: "radial-gradient(ellipse at 50% 50%, rgba(200,162,76,0.22) 0%, transparent 65%)",
              }}
            />
          )}
          ...
          >
            ...
            <path
              ...
              stroke={GOLD_DIM} strokeWidth="0.7" strokeDasharray="18 4 30 6 22 3"
            />

            <line x1="28" y1="14" x2="118" y2="42" stroke="rgba(0,0,0,0.55)" strokeWidth="1.3" strokeLinecap="round" />
            <line x1="195" y1="22" x2="252" y2="55" stroke="rgba(0,0,0,0.50)" strokeWidth="1.1" strokeLinecap="round" />
            <line x1="44"  y1="36" x2="160" y2="40" stroke="rgba(0,0,0,0.38)" strokeWidth="0.8" strokeLinecap="round" />

            <path d="M130,5 L136,8 L132,9 Z" fill="rgba(0,0,0,0.55)" />

            {([[16,11],[274,11],[16,65],[274,65]] as [number,number][]).map(([cx, cy]) => (
              <g key={`${cx}-${cy}`}>
                <circle cx={cx} cy={cy} r="2.8" fill="rgba(100,85,45,0.55)" />
                <circle cx={cx} cy={cy} r="1.4" fill="rgba(55,48,25,0.9)" />
                <circle cx={cx - 0.6} cy={cy - 0.6} r="0.5" fill="rgba(200,162,76,0.18)" />
              </g>
            ))}

            <path d="M14,11 Q10,22 12,35"   stroke="rgba(90,75,35,0.25)"  strokeWidth="2.5" strokeLinecap="round" />
            <path d="M276,65 Q280,54 278,42" stroke="rgba(80,68,30,0.2)"   strokeWidth="2"   strokeLinecap="round" />

            {(["ᚦ","ᚮ","ᚱ"] as string[]).map((r, i) => (
              <text key={r} x={20 + i * 11} y="42" textAnchor="middle" fontSize="8.5" fontFamily="serif" fill={GOLD_DIM}>{r}</text>
            ))}
            {(["ᛏ","ᚺ","ᚢ"] as string[]).map((r, i) => (
              <text key={r} x={258 + i * 11} y="42" textAnchor="middle" fontSize="8.5" fontFamily="serif" fill={GOLD_DIM}>{r}</text>
            ))}

            <text x="145" y="14" textAnchor="middle" fontSize="6" fontFamily="serif" letterSpacing="7" fill={GOLD_FAINT}>ᚠ᛫ᚢ᛫ᚦ᛫ᚨ᛫ᚱ</text>
            <text x="145" y="66" textAnchor="middle" fontSize="6" fontFamily="serif" letterSpacing="7" fill={GOLD_FAINT}>ᛇ᛫ᛈ᛫ᛉ᛫ᛊ᛫ᛏ᛫ᛒ</text>
          </svg>

          <div className="absolute inset-0 flex items-center justify-center px-8">
            <span
              className={cn(
                "font-norse font-semibold uppercase sm:text-lg tracking-[0.2em] select-none transition-all duration-300",
                glow
